...
class BeliefPropagationTorch(nn.Module):
  ...
  def forward(self, J, b, msg_node, msg_adj, target=None, mask=None):
    ...
    for ii in range(self.max_iter):
      # 1. Message Update : log(psi) + log(phi) + sum(log messages) : (E X 2 X 2) + (E X 1 X 2) + (E X 1 X 2)
      log_temp = log_psi + log_phi[edge_in].unsqueeze(dim=1) + torch.mm(msg_adj.t(), log_msg).unsqueeze(dim=1)
    ...
    return torch.exp(log_prob), loss
...
if __name__ == '__main__':
  ...
  model = BeliefPropagation(A, J, b, msg_node, msg_adj)
  P = model.inference()
